...
static void
libavcodec_destroy_encoder_context(H264_CONTEXT *WINPR_RESTRICT h264) {
...
static BOOL
libavcodec_create_encoder_context(H264_CONTEXT *WINPR_RESTRICT h264) {
    BOOL recreate = FALSE;
    H264_CONTEXT_LIBAVCODEC *sys = NULL;
    ...
    case H264_RATECONTROL_CQP:
        if (av_opt_set_int(sys->codecEncoderContext, "qp", h264->QP,
...
#ifdef WITH_VAAPI_H264_ENCODING
    if (sys->hwctx) {
        av_opt_set(sys->codecEncoderContext, "preset", "veryslow",
                   AV_OPT_SEARCH_CHILDREN);
...
#endif
    {
        av_opt_set(sys->codecEncoderContext, "preset", "medium",
                   AV_OPT_SEARCH_CHILDREN);
...
static int libavcodec_decompress(H264_CONTEXT *WINPR_RESTRICT h264,
...
#ifdef WITH_VAAPI
    status =
        avcodec_receive_frame(sys->codecDecoderContext,
    ...
    status = avcodec_decode_video2(
        sys->codecDecoderContext,
        sys->hwctx ? sys->hwVideoFrame : sys->videoFrame, &gotFrame, packet);
#else
    status = avcodec_decode_video2(sys->codecDecoderContext, sys->videoFrame,
                                   &gotFrame, packet);
#endif
#endif
    if (status < 0) {
    ...
    if (sys->hwctx) {
        if (sys->hwVideoFrame->format == sys->hw_pix_fmt) {
            ...
            status =
                av_hwframe_transfer_data(sys->videoFrame, sys->hwVideoFrame, 0);
        } else {
...
static int libavcodec_compress(H264_CONTEXT *WINPR_RESTRICT h264,
    ...
    if (sys->hwctx) {
        ...
        if ((status =
                 av_hwframe_get_buffer(sys->codecEncoderContext->hw_frames_ctx,
        ...
        if ((status = av_hwframe_transfer_data(sys->hwVideoFrame,
                                               sys->videoFrame, 0)) < 0) {
            WLog_Print(h264->log, WLOG_ERROR,
...
#ifdef WITH_VAAPI_H264_ENCODING
    status =
        avcodec_send_frame(sys->codecEncoderContext,
                           sys->hwctx ? sys->hwVideoFrame : sys->videoFrame);
#else
    ...
    if (status < 0) {
        WLog_Print(h264->log, WLOG_ERROR,
                   "Failed to encode video frame (%s [%d])", av_err2str(status),
                   status);
        goto fail;
    }
    ...
    do {
        status = avcodec_encode_video2(sys->codecEncoderContext, sys->packet,
    ...
    if (!sys->packet->data)
        ...
        status =
            avcodec_encode_video(sys->codecDecoderContext, sys->packet->data,
    ...
    *pDstSize = (UINT32)MAX(0, sys->packet->size);
    ...
    if (!gotFrame) {
        WLog_Print(h264->log, WLOG_ERROR, "Did not get frame! (%s [%d])",
                   av_err2str(status), status);
        rc = -2;
    } else
        rc = 1;
...
libavcodec_get_format(struct AVCodecContext *ctx,
        ...
        if (*p == sys->hw_pix_fmt) {
            ...
            if (sys->codecDecoderContext->active_thread_type & FF_THREAD_FRAME)
                frames->initial_pool_size +=
                    sys->codecDecoderContext->thread_count;
            ...
            sys->codecDecoderContext->hw_frames_ctx =
                av_buffer_ref(sys->hw_frames_ctx);
#endif
            return *p;
        }
    }
...
// TODO: dynamic VAAPI device acquisition
#ifdef WITH_VAAPI

        if (!sys->hwctx) {
            int ret = av_hwdevice_ctx_create(
                &sys->hwctx, AV_HWDEVICE_TYPE_VAAPI, NULL, NULL, 0);
            if (ret < 0) {
                WLog_Print(h264->log, WLOG_ERROR,
                           "Could not initialize hardware decoder, falling "
                           "back to software: %s",
                           av_err2str(ret));
                sys->hwctx = NULL;
    ...
    fail_hwdevice_create:
        ...
        if (avcodec_open2(sys->codecDecoderContext, sys->codecDecoder, NULL) <
            0) {
        ...
        if (h264->hwAccel) /* user requested hw accel */
        {
            ...
            } else if (av_hwdevice_ctx_create(&sys->hwctx,
                                              AV_HWDEVICE_TYPE_VAAPI, NULL,
                                              NULL, 0) < 0) {
                WLog_Print(h264->log, WLOG_ERROR,
...
const H264_CONTEXT_SUBSYSTEM g_Subsystem_libavcodec = {
    "libavcodec", libavcodec_init, libavcodec_uninit, libavcodec_decompress,
    libavcodec_compress};
